...
class JimengCoverProvider(BaseCoverProvider):
    """基于即梦（Jimeng）Images API 的封面生成实现"""
    ...
    async def _request_cover(
        ...
        *,
        ...
        payload: dict[str, Any] = {
            ...
            "ratio": self._get_ratio(width=width, height=height),
        ...
        logger.debug(
            "即梦 封面生成请求开始: url=%s model=%s width=%s height=%s prompt_len=%s prompt_preview=%s",
            url,
            model,
            width,
            height,
        ...
        try:
            ...
            logger.debug(
                "即梦 封面生成响应: status=%s content_type=%s body_preview=%s",
            ...
            logger.error(
                "即梦 封面生成 HTTP 错误: status=%s response=%s",
                exc.response.status_code if exc.response else None,
                exc.response.text[:2000] if exc.response is not None else None,
            )
            raise
        except Exception:
            logger.error("即梦 封面生成请求异常", exc_info=True)
            raise

        images = data.get("data") or []
        logger.debug(
            "即梦 封面生成解析结果: has_data=%s image_count=%s keys=%s",
            bool(data),
            len(images),
        ...
        if not images:
            logger.error("即梦 未返回图片结果: data=%s", data)
            raise ValueError("即梦 未返回图片结果")
        ...
        if b64_json:
            ...
            logger.debug(
                "即梦 返回 base64 图片: bytes=%s mime=image/jpeg size=%sx%s",
                len(decoded_content),
                image_width,
                image_height,
            )
        ...
        if image_url:
            logger.debug("即梦 返回图片 URL，开始下载: %s", image_url)
            try:
                async with httpx.AsyncClient(timeout=180.0) as client:
                    image_response = await client.get(image_url)

                logger.debug(
                    "即梦 图片下载响应: status=%s content_type=%s content_length=%s",
                ...
                logger.error(
                    "即梦 图片下载 HTTP 错误: status=%s response=%s",
                    exc.response.status_code if exc.response else None,
                    exc.response.text[:2000] if exc.response is not None else None,
                )
                raise
            except Exception:
                logger.error("即梦 图片下载异常", exc_info=True)
                raise
            ...
            logger.debug(
                "即梦 图片下载完成: bytes=%s extension=%s size=%sx%s",
        ...
        logger.error("即梦 返回内容中既没有 b64_json，也没有 url: %s", data)
        raise ValueError("即梦 未返回可用的图片数据")
    ...
    @staticmethod
    def _get_ratio(*, width: int, height: int) -> str:
        """即梦使用 ratio 参数，格式为 "16:9" 等"""
